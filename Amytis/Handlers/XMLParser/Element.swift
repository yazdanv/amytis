...
/**
    ...
    return `<bar></bar>` element from `<element><foo><bar></bar></foo></element>` XML as an `XML` object.
*/

import JavaScriptCore

@objc public protocol XMLJSExport: JSExport {}
...
open class XML: NSObject {
    ...
    /**
        ...
        - returns: An initialized `XML` object.
    */
    ...
    open subscript(key: String) -> XML {
        guard let
            first = children.first(where: { $0.name == key })
        else {
            let errorElement = XML(name: key)
    ...
    */
    @discardableResult open func addChild(_ child: XML) -> XML {
        child.parent = self
    ...
    */
    ...
    {
        let child = XML(name: name, value: value, attributes: attributes)
    ...
    fileprivate func removeChild(_ child: XML) {
